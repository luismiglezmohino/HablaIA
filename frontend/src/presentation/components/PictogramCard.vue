...
<template>
  <div
    ...
    class="group rounded-2xl border border-surface-200 border-t-[5px] bg-white shadow-card motion-safe:transition-all motion-safe:duration-200 motion-safe:hover:shadow-card-hover motion-safe:hover:-translate-y-1 has-[:focus-visible]:ring-4 has-[:focus-visible]:ring-primary-500 has-[:focus-visible]:ring-offset-2 has-[:focus-visible]:shadow-card-hover has-[:focus-visible]:-translate-y-1"
  >
    <button
      :aria-label="`Pictograma ${props.pictogram.label}`"
      :disabled="props.disabled"
      class="min-h-touch min-w-touch flex w-full flex-col items-center overflow-hidden rounded-2xl text-center transition-colors focus:outline-none disabled:cursor-not-allowed disabled:opacity-40"
